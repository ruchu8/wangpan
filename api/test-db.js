// Vercel Serverless Function for database testing
    ...
    try {
      ...
      if (tableNames.includes('admin_credentials')) {
        try {
          ...
          console.log('admin_credentials count:', result[0].count);
        } catch (error) {
          tableData.admin_credentials = 'Error: ' + error.message;
          console.error('Error checking admin_credentials:', error);
        }
      }
      
      if (tableNames.includes('admin_tokens')) {
        try {
          console.log('Checking admin_tokens table...');
          const result = await sql`SELECT COUNT(*) as count FROM admin_tokens`;
          tableData.admin_tokens = result[0].count;
          console.log('admin_tokens count:', result[0].count);
        } catch (error) {
          tableData.admin_tokens = 'Error: ' + error.message;
          console.error('Error checking admin_tokens:', error);
        }
      }
